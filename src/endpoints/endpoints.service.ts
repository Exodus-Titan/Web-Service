...
export class EndpointsService {
  ...
  async listFaculties() {
    const facultiesList = await this.facultyRepo.find({
      where: {
        status: 'enable',
      },
    });
  ...
  async updateFaculty(updateFacultyObject: UpdateFacultyDto) {
    const faculty = await this.facultyRepo.findOneBy({
      id: updateFacultyObject.idSearch,
    });
    if (!faculty) {
      throw new NotFoundException(
        'The faculty with the id ' +
          updateFacultyObject.idSearch.toString +
          ' not found',
      );
    }
  ...
  async deleteFaculty(id: idDto) {
    ...
    if (!faculty) {
      throw new NotFoundException(
        'The faculty with the id ' + id.toString() + ' not found',
      );
    }
  ...
  async listSchools() {
    const schollsList = await this.schoolRepo.find({
      where: {
        status: 'enable',
      },
    });
  ...
  async createSchool(createSchoolObject: CreateSchoolDto) {
    const newSchool = this.schoolRepo.create(
      createSchoolObject as DeepPartial<School>,
    ); //no c si esto vaya a funcionar
  ...
  async updateSchool(updateSchoolObject: UpdateSchoolDto) {
    const school = await this.schoolRepo.findOneBy({
      id: updateSchoolObject.idSearch,
    });
    if (!school) {
      throw new NotFoundException(
        'The school with the id ' +
          updateSchoolObject.idSearch.toString() +
          ' not found',
      );
    }
  ...
  async deleteSchool(id: idDto) {
    ...
    if (!school) {
      throw new NotFoundException(
        'The school with the id ' + id.toString() + ' not found',
      );
    }
  ...
  async listSections() {
    const sectionsList = await this.sectionRepo.find({
      where: {
        status: 'enable',
      },
    });
  ...
  async createSection(createSectionObject: CreateLessonDto) {
    const newSection = this.sectionRepo.create(
      createSectionObject as DeepPartial<Section>,
    );
  ...
  async updateSection(updateSectionObject: UpdateLessonDto) {
    const section = await this.sectionRepo.findOneBy({
      id: updateSectionObject.idSearch,
    });
    if (!section) {
      throw new NotFoundException(
        'The section with the id ' +
          updateSectionObject.idSearch.toString() +
          ' not found',
      );
    }
  ...
  async deleteSection(id: idDto) {
    ...
    if (!section) {
      throw new NotFoundException(
        'The section with the id ' + id.toString() + ' not found',
      );
    }
  ...
  async listPersons() {
    const PersonsList = await this.personRepo.find({
      where: {
        status: 'enable',
      },
    });
  ...
  async updatePerson(updatePersonObject: UpdatePersonDto) {
    const person = await this.personRepo.findOneBy({
      id: updatePersonObject.idSearch,
    });
    if (!person) {
      throw new NotFoundException(
        'The person with the id ' +
          updatePersonObject.idSearch.toString() +
          ' not found',
      );
    }
  ...
  async deletePerson(id: idDto) {
    ...
    if (!person) {
      throw new NotFoundException(
        'The person with the id ' + id.toString() + ' not found',
      );
    }
